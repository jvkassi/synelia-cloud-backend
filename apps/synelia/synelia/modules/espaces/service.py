...
depot = Depot("espace", m.EspaceCloud, libelle="Espace Cloud", champ_nom="code", champs_recherche=("code", "offreNom", "site"))
...
async def usage(ctx: Contexte, espace_id: str) -> dict[str, float]:
    vms = await Depot("vm", m.Vm).tous(ctx, filtre=lambda v: v.espaceId == espace_id and v.statut != "error")
    volumes = await Depot("volume", m.Volume).tous(ctx, filtre=lambda v: getattr(v, "espaceId", None) == espace_id)
    return {
        "vcpu": sum(v.vcpu for v in vms),
        "ramGo": sum(v.ramGo for v in vms),
        "stockageTo": round((sum(v.diskGo for v in vms) + sum(getattr(v, "tailleGo", 0) or 0 for v in volumes)) / 1024, 2),
    }


async def verifier_quota(ctx: Contexte, espace_id: str, vcpu: int = 0, ram_go: int = 0, disk_go: int = 0) -> m.EspaceCloud:
    e = await depot.obtenir(ctx, espace_id)
    u = await usage(ctx, espace_id)
    if u["vcpu"] + vcpu > e.quota.vcpu or u["ramGo"] + ram_go > e.quota.ramGo or u["stockageTo"] + disk_go / 1024 > e.quota.stockageTo:
        raise erreurs.quota_depasse("Le quota de l'Espace Cloud est atteint.", detail=f"usage={u} quota={e.quota.model_dump()}")
    return e
...
class ExecuteurEspaceCreate(Executeur):
    ...
    async def etape(self, ctx: Contexte, travail: Travail, index: int, nom: str) -> str | None:
        ...
        if index == 1:
            ...
            c["projet_id"] = a.creer_projet(c["domaine_id"], f"espace-{e.code}", "RegionOne" if e.site == "ABJ" else "GBM")
            ...
            await depot.definir_secrets(ctx, e.id, {"application_credential_id": ac["id"], "application_credential_secret": ac["secret"]})
        travail.contexte = c
        return None
